...
RendererWindow::RendererWindow(const char *window_tittle, int width,
                               int height) {
  ...
  this->window =
      SDL_CreateWindow(window_tittle, 0, 0, width, height, SDL_WINDOW_SHOWN);
  if (window == NULL) {
...
SDL_Texture *RendererWindow::loadTexture(const char *file_path) {
  ...
  // std::unique_ptr<SDL_Texture> texture_ptr =
  // std::unique_ptr<SDL_Texture>(*texture,SDL_Deleter());
  return texture;
}
...
void RendererWindow::clearScreen() { SDL_RenderClear(renderer); }
void RendererWindow::display() { SDL_RenderPresent(renderer); }
